...
class _MadmomModuleMapUnpickler(pickle.Unpickler):
    """
    Unpickler that remaps `madmom.*` module paths in upstream model pickles to
    the vendored package namespace.
    ...
    def find_class(self, module, name):
        if module == self._UPSTREAM_ROOT or module.startswith(
            f"{self._UPSTREAM_ROOT}."
        ):
...
def process_online(processor, infile, outfile, **kwargs):
    ...
    """
    from .audio.signal import Stream, FramedSignal
    # set default values
